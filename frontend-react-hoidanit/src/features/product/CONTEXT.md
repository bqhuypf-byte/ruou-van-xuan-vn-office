...
- **Variant image switching**: selecting an attribute option switches the main gallery image to the matching variant image (or the Admin-configured image for that option). Variant images are included in the thumbnail gallery and option buttons show their image swatches when available.
