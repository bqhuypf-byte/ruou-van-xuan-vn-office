...
export const BannerTable = ({ banners, isLoading, onEdit, onDelete }: BannerTableProps) => {
  if (isLoading) {
    return (
        ...
        <table className="w-full text-left text-sm border-collapse">
          <thead>
            <tr className="bg-slate-50/80 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 font-semibold border-b border-slate-200 dark:border-slate-800">
              <th className="py-3.5 px-6">Banner</th>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {banners.map((banner) => (
              <tr key={banner.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40">
