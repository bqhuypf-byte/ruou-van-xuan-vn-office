...
describe('AuthService', () => {
  ...
  describe('register', () => {
    it('should create the user with the default customer role', async () => {
      ...
      expect(usersService.create).toHaveBeenCalledWith({ ...dto, roleId: customerRole.id });
  ...
  describe('login', () => {
    ...
    it('should throw UnauthorizedException when account is inactive', async () => {
      usersService.findRawByEmail.mockResolvedValue({ ...mockUser, isActive: false });
      (bcrypt.compare as jest.Mock).mockResolvedValue(true);
  ...
  describe('refresh', () => {
    it('should rotate the refresh token and issue a new access token', async () => {
      refreshTokenRepository.findByTokenHash.mockResolvedValue(mockRefreshToken);
      usersService.findRawById.mockResolvedValue(mockUser);
      rolesService.findOne.mockResolvedValue(customerRole);

      const result = await service.refresh('raw-refresh-token', {});

      expect(refreshTokenRepository.revoke).toHaveBeenCalledWith(mockRefreshToken);
      expect(result.accessToken).toBe('signed-access-token');
      expect(result.refreshToken).toEqual(expect.any(String));
    });
  ...
  describe('changePassword', () => {
    it('should hash and store the new password, then revoke all refresh tokens', async () => {
      ...
      expect(usersService.setPasswordHash).toHaveBeenCalledWith(1, 'new-hashed-password');
